...
const EXPECTED_STATS_FILE = path.join(EXPECTED_STATS_DIR, 'compression-stats.json');
...
async function testHardcodedPaths() {
  ...
  try {
    ...
    const statsFileMatch = serverSource.match(/const STATS_FILE = path\.join\(STATS_DIR, 'compression-stats\.json'\);/);
...
async function testEnvironmentVariableNotUsed() {
  ...
  try {
    ...
    if (!envVarMatch) {
      console.log('  ✅ No environment variable checks for stats file path found');
      console.log('     Confirmed: Server does NOT support UCPL_STATS_FILE or similar env vars');
      return true;
    } else {
...
async function testStatsFileUsedDirectly() {
  ...
  try {
    ...
    const saveStatsMatch = serverSource.match(/await fs\.writeFile\(STATS_FILE,/);
...
async function testDocumentation() {
  ...
  try {
    ...
    const statsPathDoc = readme.match(/~\/\.ucpl\/compress\/compression-stats\.json/);
    ...
    if (statsPathDoc && hardcodedNote) {
      ...
      console.log('     - Statistics file path: ~/.ucpl/compress/compression-stats.json');
...
async function testNoUnusedMocks() {
  ...
  try {
    ...
    const testFile = await fs.readFile(path.join(__dirname, 'test-statistics-fallback.js'), 'utf-8');

    // Verify UCPL_STATS_FILE is NOT set
    const envVarSet = testFile.match(/process\.env\.UCPL_STATS_FILE\s*=/);

    if (!envVarSet) {
      console.log('  ✅ test-statistics-fallback.js does NOT mock UCPL_STATS_FILE');
      console.log('     Test uses its own temporary file for isolation');
      return true;
    } else {
      console.log('  ❌ Found unused environment variable mock:');
      console.log('     test-statistics-fallback.js sets UCPL_STATS_FILE (not used by server)');
      return false;
    }
  } catch (error) {
    console.log(`  ❌ Test failed: ${error.message}`);
...
async function runTests() {
  ...
  if (passed === total) {
    ...
    console.log('\n📝 Summary:');
    console.log('   - Statistics file path is hardcoded: ~/.ucpl/compress/compression-stats.json');
    ...
    console.log('   - Documentation accurately reflects configuration behavior');
    process.exit(0);
  } else {
