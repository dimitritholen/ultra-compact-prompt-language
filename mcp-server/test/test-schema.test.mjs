...
describe('MCP Server Schema Validation', () => {
  ...
  describe('Description Length', () => {
    test('should have description within 255 character limit', () => {
      ...
      assert.ok(descLength <= 255, `Description length ${descLength} exceeds 255 character limit`);
    });
  });
  ...
  describe('Parameter Validation Constraints', () => {
    ...
    test('path should have minLength constraint', () => {
      assert.ok(props.path.minLength !== undefined, 'path should have minLength');
    });

    test('path should have maxLength constraint', () => {
      assert.ok(props.path.maxLength !== undefined, 'path should have maxLength');
    });
    ...
    test('language should have oneOf constraint', () => {
      assert.ok(props.language.oneOf !== undefined, 'language should have oneOf');
    });
    ...
    test('format should have default value', () => {
      assert.ok(props.format.default !== undefined, 'format should have default');
    });

    test('include should have items schema', () => {
      assert.ok(props.include.items !== undefined, 'include should have items schema');
    });

    test('include should have minItems constraint', () => {
      assert.ok(props.include.minItems !== undefined, 'include should have minItems');
    });

    test('exclude should have items schema', () => {
      assert.ok(props.exclude.items !== undefined, 'exclude should have items schema');
    });

    test('exclude should have minItems constraint', () => {
      assert.ok(props.exclude.minItems !== undefined, 'exclude should have minItems');
    });
    ...
    test('offset should have minimum constraint', () => {
      assert.ok(props.offset.minimum !== undefined, 'offset should have minimum');
    });

    test('offset should have default value', () => {
      assert.ok(props.offset.default !== undefined, 'offset should have default');
    });
  });
  ...
  describe('Enum to oneOf Conversion', () => {
    ...
    test('level should use oneOf instead of enum', () => {
      assert.ok(props.level.oneOf !== undefined, 'level should have oneOf');
      assert.strictEqual(props.level.enum, undefined, 'level should not have enum');
    });

    test('language should use oneOf instead of enum', () => {
      assert.ok(props.language.oneOf !== undefined, 'language should have oneOf');
      assert.strictEqual(props.language.enum, undefined, 'language should not have enum');
    });

    test('format should use oneOf instead of enum', () => {
      assert.ok(props.format.oneOf !== undefined, 'format should have oneOf');
      assert.strictEqual(props.format.enum, undefined, 'format should not have enum');
    });
  });

  describe('Output Schema', () => {
    test('should have outputSchema defined', () => {
      assert.ok(tool.outputSchema !== undefined, 'Tool should have outputSchema');
    });

    test('outputSchema should have properties', () => {
      if (tool.outputSchema) {
        const props = Object.keys(tool.outputSchema.properties || {});
        assert.ok(props.length > 0, 'outputSchema should have properties defined');
      }
    });
  });
  ...
  describe('Tool Annotations', () => {
    ...
    test('should have audience annotation', () => {
      if (tool.annotations) {
        assert.ok(tool.annotations.audience !== undefined, 'Should have audience annotation');
      }
    });

    test('should have priority annotation', () => {
      if (tool.annotations) {
        assert.ok(tool.annotations.priority !== undefined, 'Should have priority annotation');
      }
    });

    test('should have readOnlyHint annotation', () => {
      if (tool.annotations) {
        assert.ok(tool.annotations.readOnlyHint !== undefined, 'Should have readOnlyHint annotation');
      }
    });

    test('should have destructiveHint annotation', () => {
      if (tool.annotations) {
        assert.ok(tool.annotations.destructiveHint !== undefined, 'Should have destructiveHint annotation');
      }
    });

    test('should have openWorldHint annotation', () => {
      if (tool.annotations) {
        assert.ok(tool.annotations.openWorldHint !== undefined, 'Should have openWorldHint annotation');
      }
    });
  });
  ...
  describe('Parameter Description Lengths', () => {
    ...
    test('all parameter descriptions should be reasonable length', (t) => {
      for (const [param, schema] of Object.entries(props)) {
        ...
        if (length > 150) {
          t.diagnostic(`  Warning: ${param} description is longer than recommended (${length} > 150)`);
        }
      }
  ...
  describe('Discoverability Score', () => {
    test('should calculate and verify discoverability score', (t) => {
      ...
      t.diagnostic(`Discoverability Score: ${score}/${maxScore} (${percentage}%)`);

      assert.ok(score >= maxScore * 0.8, `Score ${score}/${maxScore} should be at least 80%`);
    });
  });
});
