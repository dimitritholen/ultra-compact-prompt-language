...
describe("Cost Tracking Integration", () => {
  ...
  describe("Cost fields in compression records", () => {
    test("should include all cost fields in compression record", async () => {
      const originalContent =
        'function test() { return "hello world"; }'.repeat(100);
      ...
      assert.ok(
        record.costSavingsUSD >= 0,
        "costSavingsUSD should be non-negative",
      );
    });
  });
  ...
  describe("LLM detection caching", () => {
    test("should handle multiple compressions with consistent cost fields", async () => {
      ...
      const record1 = await recordCompression(
        TEST_FILE,
        originalContent,
        compressedContent,
        "full",
      ...
      const record2 = await recordCompression(
        TEST_FILE,
        originalContent,
        compressedContent,
        "full",
      ...
      const record3 = await recordCompression(
        TEST_FILE,
        originalContent,
        compressedContent,
        "full",
      ...
      assert.strictEqual(
        record1.client,
        record2.client,
        "Clients should be consistent",
      );
      assert.strictEqual(
        record1.pricePerMTok,
        record2.pricePerMTok,
  ...
  describe("Backward compatibility", () => {
    test("should maintain backward compatibility with old stats", async () => {
      ...
      assert.strictEqual(
        loadedStats.recent[0].model,
        undefined,
        "Old record should not have model field",
      );
      ...
      assert.strictEqual(
        updatedStats.recent.length,
        2,
        "Should have 2 records",
      );
      assert.strictEqual(
        updatedStats.recent[0].model,
        undefined,
        "Old record still has no model field",
      );
      assert.ok(
        updatedStats.recent[1].model !== undefined,
        "New record should have model field",
      );
    });
  });
  ...
  describe("Graceful error handling", () => {
    test("should handle pricing failures gracefully", async () => {
      ...
      assert.strictEqual(
        resultNegative.costSavingsUSD,
        0,
        "Should return 0 cost for negative tokens",
      );
      assert.strictEqual(
        resultNegative.model,
        DEFAULT_MODEL,
        "Should return default model on error",
      );
      assert.strictEqual(
        typeof resultNegative.pricePerMTok,
        "number",
      ...
      assert.strictEqual(
        resultNaN.costSavingsUSD,
        0,
        "Should return 0 cost for NaN tokens",
      );
      assert.strictEqual(
        resultNaN.model,
        DEFAULT_MODEL,
      ...
      assert.strictEqual(
        resultString.costSavingsUSD,
        0,
        "Should return 0 cost for non-number tokens",
      );
      assert.strictEqual(
        resultString.model,
        DEFAULT_MODEL,
      ...
      const resultUnknownModel = await calculateCostSavings(
        1000,
        "totally-fake-model-xyz",
      );
      assert.ok(
        resultUnknownModel.costSavingsUSD >= 0,
        "Should calculate cost with default pricing",
      );
      assert.strictEqual(
        resultUnknownModel.model,
        "totally-fake-model-xyz",
        "Should preserve model name",
      );
      assert.ok(
        typeof resultUnknownModel.pricePerMTok === "number",
