...
describe("Cost Calculation", () => {
  describe("Specific Models", () => {
    ...
    test("GPT-4o", async () => {
      ...
      assert.strictEqual(result.costSavingsUSD, 5.0);
      assert.strictEqual(result.model, "gpt-4o");
      assert.strictEqual(result.pricePerMTok, 2.5);
    });
  });
  ...
  describe("Edge Cases", () => {
    ...
    test("Auto-detect model", async () => {
      ...
      assert.strictEqual(result.model, "claude-opus-4");
      assert.strictEqual(result.costSavingsUSD, 15.0);
      clearEnvVars();
    });
    ...
    test("Invalid model fallback", async () => {
      ...
      assert.strictEqual(result.costSavingsUSD, 3.0); // Uses default pricing
    });
  });
});
