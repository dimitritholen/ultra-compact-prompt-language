...
describe("Multi-tier Statistics Retention", () => {
  describe("Migration from old format to new format", () => {
    test("should migrate compressions to correct tiers", (t) => {
      ...
      t.diagnostic(
        `Daily aggregates: ${Object.keys(newStats.daily).length} (expected: 2)`,
      );
      t.diagnostic(
        `Monthly aggregates: ${Object.keys(newStats.monthly).length} (expected: 1)`,
      );

      assert.strictEqual(
        newStats.recent.length,
        2,
        "Should have 2 recent records",
      );
      assert.strictEqual(
        Object.keys(newStats.daily).length,
        2,
        "Should have 2 daily aggregates",
      );
      assert.strictEqual(
        Object.keys(newStats.monthly).length,
        1,
        "Should have 1 monthly aggregate",
      );
      assert.strictEqual(
        newStats.summary.totalCompressions,
        5,
  ...
  describe("Auto-aggregation on save", () => {
    test("should move old records to appropriate tiers", (t) => {
      ...
      t.diagnostic(
        `Recent after aggregation: ${aggregated.recent.length} (expected: 2)`,
      );
      t.diagnostic(
        `Daily after aggregation: ${Object.keys(aggregated.daily).length} (expected: 1)`,
      );
      t.diagnostic(
        `Monthly after aggregation: ${Object.keys(aggregated.monthly).length} (expected: 1)`,
      );

      assert.strictEqual(
        aggregated.recent.length,
        2,
        "Should keep 2 recent records",
      );
      assert.strictEqual(
        Object.keys(aggregated.daily).length,
        1,
        "Should have 1 daily aggregate",
      );
      assert.strictEqual(
  ...
  describe("Retention policy enforcement", () => {
    test("should prune old data according to policy", (t) => {
      const stats = {
        ...
        daily: {
          "2021-06-01": {
            // ~4.4 years old - should be moved to monthly (within 5y retention)
            date: "2021-06-01",
            count: 10,
        ...
        monthly: {
          "2018-01": {
            // ~7 years old - older than 5 years - should be pruned
            month: "2018-01",
            count: 50,
      ...
      t.diagnostic(
        `- Monthly keys: ${Object.keys(aggregated.monthly).join(", ")}`,
      );
      ...
      assert.strictEqual(
        has2018,
        false,
        "2018-01 should be pruned (older than 5 years)",
      );
      ...
      assert.strictEqual(
        has2021,
        true,
        "2021-06 should be in monthly (within 5y retention)",
      );
    });
  });
    ...
    test("should maintain bounded size with retention policy", (t) => {
      ...
      for (let day = 0; day < 30; day++) {
        for (let i = 0; i < 1000; i++) {
          recentCompressions.push(
            createMockCompression(day, `file-${day}-${i}.js`),
          );
        }
      }
      ...
      t.diagnostic(
        `Uncompressed stats size: ${Math.round(uncompressedSize / 1024)} KB`,
      );
      ...
      t.diagnostic(
        `After aggregation: ${Math.round(aggregatedSize / 1024)} KB`,
      );
      t.diagnostic(
        `Recent records after aggregation: ${aggregated.recent.length}`,
      );
      ...
      t.diagnostic(
        `Total records after aggregation: ${totalRecords} (max: ${maxExpectedRecords})`,
      );

      assert.ok(
        totalRecords <= maxExpectedRecords,
        "Total records should be within bounds",
      );
    });
  });
});
