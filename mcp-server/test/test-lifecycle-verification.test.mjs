...
async function teardownTest() {
  ...
  try {
    ...
    if (exists) {
      console.log(
        `  [afterEach] Teardown ${teardownCount}: Cleanup running (file exists)`,
      );
    } else {
      console.log(
        `  [afterEach] Teardown ${teardownCount}: Cleanup running (file already removed)`,
      );
    }
...
async function verifyCleanup() {
  ...
  try {
    ...
    console.log(
      "  ❌ FAILED: Test directory still exists - cleanup did NOT run!",
    );
    return false;
  } catch (error) {
    console.log(
      "  ✅ SUCCESS: Test directory removed - cleanup ran correctly!",
    );
    return true;
  }
}
...
async function runTests() {
  ...
  console.log(
    "║  Lifecycle Hook Verification Tests                          ║",
  );
  console.log(
    "║  Demonstrates: afterEach runs even on test failure          ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════╝",
  );
