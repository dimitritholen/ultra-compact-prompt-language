...
describe("Integration Coverage - Full Workflows", () => {
  ...
  before(async () => {
    TEST_DIR = path.join(
      os.tmpdir(),
      `test-integration-coverage-${Date.now()}`,
    );
    ...
    test("compress with all format options", async () => {
      ...
      for (const format of ["text", "summary", "json"]) {
        ...
        assert.ok(
          ...
          `Format ${format} should have content`,
        );
        ...
        if (format === "json") {
          ...
          assert.ok(
            Array.isArray(response.result.content) ||
              typeof response.result.content === "object",
  ...
  describe("Compression Level Options", () => {
    test("compress with all compression levels", async () => {
      ...
      for (const level of ["minimal", "signatures", "full"]) {
        ...
        assert.ok(
          response.result.content,
          `Level ${level} should have content`,
        );
        ...
        assert.ok(
          ...
          `Level ${level} should return string content`,
        );
      }
    });
  });
  ...
  describe("Statistics Period Options", () => {
    test("get_compression_stats with all period options", async () => {
      ...
      for (const period of ["all", "today", "week", "month"]) {
        ...
        assert.ok(
          response.result.content,
          `Period ${period} should have content`,
        );
        ...
        assert.ok(
          typeof content === "string",
          `Period ${period} should return string content`,
        );
      }
    });
  });
  ...
  describe("Statistics Date Range Queries", () => {
    test("get_compression_stats with date ranges", async () => {
      ...
      assert.ok(
        response.result.content,
        "Date range query should have content",
      );
    });
    ...
    test("get_compression_stats with relative days", async () => {
      ...
      assert.ok(
        response.result.content,
        "Relative days query should have content",
      );
    });
  });
  ...
  describe("Error Scenarios", () => {
    test("compress non-existent file returns error", async () => {
      ...
      assert.ok(
        response.error || (response.result && response.result.isError),
    ...
    test("compress with invalid level falls back gracefully", async () => {
      ...
      assert.ok(
        ...
        "Invalid level should be handled",
      );
    });
    ...
    test("get_compression_stats with invalid date range", async () => {
      ...
      assert.ok(
        response.result || response.error,
        "Invalid dates should be handled",
      );
    });
  });
  ...
  describe("Combined Parameter Workflows", () => {
    test("compress with include/exclude patterns", async () => {
      ...
      assert.ok(
        response.result.content,
        "Include pattern should return content",
      );
    });
    ...
    test("compress directory with pagination", async () => {
      ...
      // First page
      const { response: page1 } = await callMCPTool(
        SERVER_PATH,
        "compress_code_context",
        {
          path: testDir,
      ...
      // Second page
      const { response: page2 } = await callMCPTool(
        SERVER_PATH,
        "compress_code_context",
        {
          path: testDir,
