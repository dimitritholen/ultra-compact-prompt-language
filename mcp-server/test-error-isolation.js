...
async function testAllTestsExecute() {
  ...
  console.log('✓ Test Results section present:', stdout.includes('Test Results:'));
  console.log('✓ Pass/Fail counts shown:', /Passed: \d+\/\d+/.test(stdout));

  const pass = allTestsRan &&
...
async function testExitCodeReflectsResults() {
  ...
  console.log('✓ Exit code matches result:',
...
async function testFailureReporting() {
  ...
  const pass = hasFailureCount && hasPassCount && stdout.includes('Test Results:');
