...
describe('Schema Validation for get_compression_stats', () => {
  test('Tool name matches regex', () => {
    ...
    assert.ok(nameValid, `Tool name "${schema.name}" should match regex pattern`);
  });
  ...
  describe('Parameter Definitions', () => {
    ...
    test('All parameters have description', () => {
      for (const [paramName, param] of Object.entries(props)) {
        assert.ok(param.description, `Parameter "${paramName}" should have a description`);
        assert.ok(param.description.length > 0, `Parameter "${paramName}" description should not be empty`);
      }
    });
    ...
    test('Optional parameters have defaults or are marked optional', () => {
      for (const [paramName, param] of Object.entries(props)) {
        ...
        const isInRequired = schema.inputSchema.required?.includes(paramName) || false;

        // If not required, it's optional (which is fine for MCP)
        assert.ok(hasDefault || !isInRequired,
  ...
  describe('Validation Constraints', () => {
    ...
    test('relativeDays has minimum constraint', () => {
      assert.strictEqual(props.relativeDays.minimum, 1,
        'relativeDays should have minimum of 1');
    });

    test('relativeDays has maximum constraint', () => {
      assert.strictEqual(props.relativeDays.maximum, 365,
        'relativeDays should have maximum of 365');
    });

    test('limit has valid constraints', () => {
      assert.strictEqual(props.limit.minimum, 1, 'limit should have minimum of 1');
      assert.strictEqual(props.limit.maximum, 100, 'limit should have maximum of 100');
    });
  });
  ...
  describe('Backward Compatibility', () => {
    ...
    test('period parameter exists', () => {
      assert.ok(props.period, 'period parameter should exist for backward compatibility');
    });

    test('period has default', () => {
      assert.strictEqual(props.period.default, 'all',
        'period should have default value "all"');
    });

    test('period has oneOf options', () => {
      assert.ok(Array.isArray(props.period.oneOf),
        'period should have oneOf array');
      assert.strictEqual(props.period.oneOf.length, 4,
